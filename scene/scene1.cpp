...
int main(){
	...
	add(0,48,32,8,0.2,2.4,0.2,1.6,0.1,0.4);
	...
	float hh=0.8;
	add2(vec3{2.2,1,hh},0.25,{vec3{-1,0,0}});
	int lala=5;
	for(int i=0;i<lala;++i)add3(vec3{2.2-0.3-0.12-i*0.12,1,hh},i&1?vec3{0,0,1}:vec3{0,1,0},vec3{1,0,0},i&1?1:3);
	add2(vec3{2.2-0.3-lala*0.12-0.17-0.25,1,hh},0.25,{vec3{1,0,0}});
	...
	for(int fr=0;fr<240;){
		...
		if(rt<=0){
			if(fr%2==0){
				int oldn=A.n;
				add(0,8,8,2,1.3,0.4,0.2,0.4,1.6,0.15);
				for(int i=oldn+1;i<=A.n;++i)A.v[i]=vec3{0,0,-2},time_p[i]=fr;
			}
			if(fr%20==15 && fr<240){
				int oldn=A.n;
				work(vec3{1.5,1.6,1.6},fr/20%2==1?1:3);
